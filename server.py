...
def connect():
    ...
    global mail
    log(DEBUG, ['Server: ', config.config['AUTHENTICATION']['server']])
    mail = imaplib.IMAP4_SSL(config.config['AUTHENTICATION']['server'])
...
def retrieve_message(email_uid):
    ...
    if data_two[0] is not None:
        ...
        raw_body = "None"
    return parser.parse_email(raw_email, raw_body)
