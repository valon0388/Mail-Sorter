...
def remove_label(uid, label):
    ...
    try:
        if options[choice]:
            ...
            response = server.mail.uid('FETCH', uid, '(X-GM-LABELS)')
            log(INFO, response)
            log(INFO, 'REMOVE LABEL')
            response = server.mail.uid('STORE', uid, '-X-GM-LABELS', label)
            log(INFO, response)
            log(INFO, 'LISTING LABELS FOR UID: {}'.format(uid))
            response = server.mail.uid('FETCH', uid, '(X-GM-LABELS)')
            log(INFO, response)
...
def move_message(uid, destination):
    ...
    try:
        ...
        if options[choice]:
            ...
            response = server.mail.uid('COPY', uid, destination)
            log(INFO, response)
            if response[0] == 'OK':
                log(INFO, server.mail.uid('STORE', uid, '+FLAGS', '(\\Deleted)'))
                log(INFO, 'ORIGINAL DELETED')
                log(INFO, server.mail.expunge())
                log(INFO, "MAIL MOVED")
            else:
